...
//     dynamic: {
//         visibility: boolean | ((info: FfprobeData) => boolean)
//         enabled: boolean | ((info: FfprobeData) => boolean)
//     }
//     run: (ffmpeg: FfmpegCommand, info: FfprobeData) => FfmpegCommand

//     type: 'checkbox'
// }

// export interface EditorOptions {
//     [key: string]: EditorOption
// }

// export interface EditorOptionsCategorized {
//     basic: EditorOptions
//     advance: EditorOptions
// }
...
const options = {
    basic: {
        fixmic: {
            ...
            run: (ffmpeg: FfmpegCommand) =>
                ffmpeg
                    .complexFilter([
                        {
                            filter: 'afftdn',
                            options: { nt: 'w', om: 'o', tr: '' },
                            inputs: '[0:a:1]',
                            outputs: 'a',
                        },
                        {
                            filter: 'agate',
                            options: { threshold: '.035' },
                            inputs: 'a',
                            outputs: 'a',
                        },
                        {
        ...
        normalize: {
            ...
            run: (ffmpeg, info) => {
                const ffmpegData = info.rawffmpeg([
                    '-i',
                    info.getInputPath(),
                    '-af',
                    'volumedetect',
                ...
                shellout = shellout.filter((l) =>
                    l.startsWith('[Parsed_volumedetect')
                )
                ...
                if ('max_volume' in audioInfo) {
                    const maxVolume = parseFloat(audioInfo.max_volume)
                    const volumeChange = -0.1 - maxVolume // -0.1 offset so max isn't at 0dB exactly

                    console.log(`Normalizing Audio (${volumeChange}dB)`)

                    if (volumeChange === 0) return ffmpeg
        ...
        tocompress: {
            ...
            run: (ffmpeg, info) => {
                ...
                const getAuto = (maxbitrate) => {
                    let size = Math.min(
                        info.data.streams.primary.video.width,
                        info.data.streams.primary.video.height
                    )
                    ...
                    if (info.data.streams.primary.video.framerate <= 30) {
                        if (size <= 360) br = 1
                        else if (size <= 480) br = 2.5
                        else if (size <= 720) br = 5
                        else if (size <= 1080) br = 8
                        else if (size <= 1440) br = 16
                        else br = 35
                    } else {
                        if (size <= 360) br = 1.5
                        else if (size <= 480) br = 4
                        else if (size <= 720) br = 7.5
                        else if (size <= 1080) br = 12
                        else if (size <= 1440) br = 24
                        else br = 55
                    }

                    bitrate = br * 1e3
                    if (maxbitrate) {
                        if (bitrate > maxbitrate) usedMaxbitrate = true
                        bitrate = Math.min(bitrate, maxbitrate)
                    }
                    bitrate = Math.min(
                        bitrate,
                        info.data.streams.primary.video.bitrate / 1000
                    )
                }
                ...
                switch (info.options.basic.compresstype) {
                    ...
                    case 'discord':
                        let max = 8
                        if (info.options.basic.discordtypeclassic) max = 50
                        else if (info.options.basic.discordtypenitro) max = 100
                ...
                if (usedMaxbitrate) videobitrate = bitrate - audiobitrate
...
audio bitrate: ${audiobitrate}Kbps`)
        ...
        discordtypenitro: {
            ...
            on: (target, info) =>
                (info.settings.discordnitro = target.checked ? 1 : 0),
            small: true,
            label: 'Nitro',
            dynamic: {
                visibility: (info) =>
                    info.options.basic.tocompress &&
                    info.options.basic.compresstype === 'discord',
        ...
        discordtypeclassic: {
            ...
            on: (target, info) =>
                (info.settings.discordnitro = target.checked ? 2 : 0),
            small: true,
            label: 'Nitro Classic',
            dynamic: {
                visibility: (info) =>
    ...
    advance: {
        // not yet implemented, this is just the idea for later
        autodiscordbuffer: {
            parent: null,
            type: 'number',
            input: {
