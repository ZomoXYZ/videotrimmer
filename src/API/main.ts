...
function getAppDataPath() {
  switch (process.platform) {
    case "darwin":
      ...
      else throw "Missing Environmental Variable $HOME";
    case "win32":
      if (process.env.APPDATA) return path.join(process.env.APPDATA, AppName);
      else throw "Missing Environmental Variable $APPDTA`";
    case "linux":
      if (process.env.HOME) return path.join(process.env.HOME, "." + AppName);
      else throw "Missing Environmental Variable $HOME";
    default:
...
function downloadFfmpeg() {
  return new Promise((complete, _error) => {
    if (
      !fs.existsSync(
        path.join(getAppDataPath(), "ffmpeg-binaries", "ffmpeg")
      ) ||
      !fs.existsSync(path.join(getAppDataPath(), "ffmpeg-binaries", "ffprobe"))
    )
      downloadBinaries(
        ["ffmpeg", "ffprobe"],
        {
